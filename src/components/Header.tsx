...
const NavLink: React.FC<{ to: string }> = ({ children, to }) => (
    <Link to={to} className="text-lg text-gray-500 hover:underline w-full ml-2">{children}</Link>
)

const Header: React.FC<{className?: string}> = ({className}) => {
    return (
        <header className={"flex justify-between items-baseline border-b pb-3" + ` ${className}`}>
